...
def data_over_time(df,col):
  ...
  nations_over_time.rename(columns={'Year':'Edition','count':f'No of {col}'},inplace=True)
...
def year_wise_medal_tally(df,country):
  ...
  last_df = new_df.groupby('Year').count()['Medal'].reset_index()
...
def country_event_heatmap(df,country):
  ...
  temp_df.drop_duplicates(subset =['Team','NOC','Games','Year','City','Sport','Event','Medal'],inplace=True)
  new_df = temp_df[temp_df['region'] == country]
  x = new_df.drop_duplicates(['Year','Sport','Event'])
...
def most_successful_countrywise(df,country):
    ...
    x = x.merge(df, on='Name', how='left')[['Name', 'Medals', 'Sport']]
